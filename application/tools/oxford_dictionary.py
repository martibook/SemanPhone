...
def extract_definitions(j):
    ...
    definitions = [(definition, sense[1]) for sense in sense_list if "definitions" in sense[0] for definition in sense[0]["definitions"]]
    # change format for the database
    definitions = [d[0] + '#' + d[1] for d in definitions]
    return definitions
